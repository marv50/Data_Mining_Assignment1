...
def create_summary_table_visualization(summary_table, filename='summary_table.png'):
    ...
    table = plt.table(
        ...
        cellColours=[['#e6f7ff'] *
                     len(summary_table.columns)] * len(summary_table)
    )
...
def create_summary_dashboard(summary_table, filename='summary_dashboard.png'):
    ...
    summary_by_entries = summary_table.sort_values(
        'Num Entries', ascending=False)

    # Plot 1: Number of entries by variable
    axes[0].bar(summary_by_entries['Variable'],
                summary_by_entries['Num Entries'], color='steelblue')
    axes[0].set_title('Number of Entries by Variable')
    axes[0].set_ylabel('Count')
    axes[0].tick_params(axis='x', rotation=90)

    # Plot 2: Missing values by variable
    axes[1].bar(summary_by_entries['Variable'],
                summary_by_entries['Num Missing Values'], color='tomato')
    axes[1].set_title('Missing Values by Variable')
    axes[1].set_ylabel('Count')
    axes[1].tick_params(axis='x', rotation=90)

    # Plot 3: Outliers by variable
    axes[2].bar(summary_by_entries['Variable'],
                summary_by_entries['Num Outliers (IQR)'], color='mediumseagreen')
...
def plot_histogram(variable, filename='histogram.png'):
    plt.figure(figsize=(10, 6))
    sns.histplot(variable, bins=30)
...
def plot_boxplot(variable, filename='boxplot.png'):
    plt.figure(figsize=(10, 6))
...
if __name__ == "__main__":
    ...
    create_summary_table_visualization(
        summary_table, filename='summary_table_STD.png')

    arousal = df.loc[df['variable'] == 'circumplex.arousal', 'value']
    valence = df.loc[df['variable'] == 'circumplex.valence', 'value']
    sns.histplot(arousal)
    sns.histplot(valence)
    plt.show()